 ...
 */
public class EndStonecuttingRecipeProvider extends RecipeProvider {
    ...
    public EndStonecuttingRecipeProvider(DataGenerator gen) {
        super(gen);
    }
            ...
            for (Pair<Item, Integer> result : results) {
                ...
                if (!flushed.contains(name)) {
                    SingleItemRecipeBuilder.stonecuttingRecipe(Ingredient.fromItems(from), to, count)
                                           ...
                                           .build(consumer, EndymionMod.resLoc(name));
                    flushed.add(name);
                }
            }
        }
